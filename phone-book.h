...
private:
  // сравнение по возрастанию имени, убыванию длительности, возрастанию номера
  struct compare_name_duration_number {
    bool operator()(const user_info_t &left, const user_info_t &right) const;
  };
  // сравнение по убыванию длительности, возрастанию имени, возрастанию номера
  struct compare_duration_name_number {
    bool operator()(const user_info_t &left, const user_info_t &right) const;
  };
  // абоненты
  std::set<user_info_t, compare_name_duration_number> users;
  // префиксы номеров и абоненты
  std::map<std::string, std::set<user_info_t, compare_duration_name_number>> prefixes;
  // звонки
